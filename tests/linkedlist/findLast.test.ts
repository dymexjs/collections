...
describe("findLast", () => {
  function verifyFind(list: LinkedList<number>, items: Array<number>): void {
    for (const item of items) {
      const node = list.findLast(item);
      assert.ok(node, `Expected to findLast item ${item}`);
  ...
  test("empty list", () => {
    ...
    assert.strictEqual(list.findLast(headItems[0]), undefined);
  });

  test("single item", () => {
    const list = new LinkedList<number>();
    list.addLast(headItems[0]);
    assert.strictEqual(list.findLast(headItems[1]), undefined);
  ...
  test("two items", () => {
    ...
    assert.strictEqual(list.findLast(headItems[2]), undefined);
  ...
  test("three items", () => {
    ...
    assert.strictEqual(list.findLast(headItems[3]), undefined);
  ...
  test("multiple items", () => {
    ...
    assert.strictEqual(list.findLast(tailItems[0]), undefined);
  ...
  test("undefined in middle", () => {
    ...
    assert.strictEqual(list.findLast(-1), undefined);
